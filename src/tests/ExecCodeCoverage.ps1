...
if (!(Test-Path -path $basedir/CoverageHistory)) {  
    New-Item -ItemType directory -Path $basedir/CoverageHistory
}
...
foreach ($project in $projects) {
    ...
    $output = [string] (& dotnet test $dir --collect:"XPlat Code Coverage" 2>&1)
    ...
    Remove-Item -Recurse -Force $basedir/coveragereport/$baseName
    
    $coverageList += "$dir/TestResults/$cmdGuid/coverage.cobertura.xml;"
}

# Generate the Code Coverage HTML Report
reportgenerator -reports:"$coverageList" -targetdir:"$dir/coveragereport" -reporttypes:Html -historydir:$dir/CoverageHistory 
